...
import matplotlib.patheffects as path_effects # <--- IMPORTACIÓN CORRECTA
import glob
import os
...
if selected_csv_name and selected_sex_display:
    ...
    if df_processed_csv is not None and not df_processed_csv.empty:
        ...
        fig, ax_main = plt.subplots(1, 1, figsize=(10, 8.5))
        ...
        if vmin is not None and vmax is not None : 
            ...
            for idx, row in peninsula_baleares_ceuta_melilla_gdf.iterrows():
                if pd.notna(row['total']) and 'rep_point' in row and row['rep_point']: # Verificar que rep_point existe
                    text_color = 'white' if row['total'] > umbral_contraste else 'black'
                    ax_main.text(row['rep_point'].x, row['rep_point'].y, 
                                 f"{row['total']:.1f}%", 
                                 fontsize=7, ha='center', va='center', color=text_color,
                                 # USO CORRECTO DE path_effects
                ...
                for idx, row in canarias_gdf.iterrows():
                    if pd.notna(row['total']) and 'rep_point' in row and row['rep_point']: # Verificar que rep_point existe
                        ...
                        ax_canarias.text(row['rep_point'].x, row['rep_point'].y, 
                                         f"{row['total']:.1f}%", 
                                         fontsize=6, ha='center', va='center', color=text_color_canarias,
                                         # USO CORRECTO DE path_effects
